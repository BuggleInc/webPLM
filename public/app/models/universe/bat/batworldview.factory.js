    ...
    function BatWorldView($compile) {
        ...
        function draw(domElt, BatWorld) {
            if(!domElt.attr('data-batworldview')) {
                ...
                var html = '<span ng-repeat="test in exercise.currentWorld.visibleTests" ng-class="{ passed: test.correct, failed: test.answered && !test.correct }">{{test.toString()}}<br></span>';
