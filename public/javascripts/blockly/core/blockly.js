...
Blockly.setMainWorkspaceMetrics_ = function (xyRatio) {
    if (xyRatio.x == Number.POSITIVE_INFINITY || xyRatio.x == Number.NEGATIVE_INFINITY)
        xyRatio.x = 0;
    if (xyRatio.y == Number.POSITIVE_INFINITY || xyRatio.y == Number.NEGATIVE_INFINITY)
        xyRatio.y = 0;
    if (!Blockly.mainWorkspace.scrollbar)
    ...
    var translation = 'translate(' + (Blockly.mainWorkspace.scrollX + metrics.absoluteLeft) + ',' + (Blockly.mainWorkspace.scrollY + metrics.absoluteTop) + ')';
